...
/// Initializes the embedded draco decoder wasm module in the CURRENT context
/// (no worker involved). Safe to call concurrently; the module is created once.
export function initDracoDecoder() {
    if (!decoderModulePromise) {
        decoderModulePromise = createDecoderModule({
            locateFile: (file) => {
